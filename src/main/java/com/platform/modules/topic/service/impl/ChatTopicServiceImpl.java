...
public class ChatTopicServiceImpl extends BaseServiceImpl<ChatTopic> implements ChatTopicService {
    ...
    public void like(Long topicId) {
        ...
        if (topicLike != null) {
            ...
            if (!YesOrNoEnum.YES.equals(topicLike.getHasLike())) {
                chatTopicLikeService.updateById(new ChatTopicLike().setId(topicLike.getId()).setHasLike(YesOrNoEnum.YES));
            }
            return;
        }
        // 点赞操作
        chatTopicLikeService.add(new ChatTopicLike().setTopicId(topicId).setUserId(userId).setHasLike(YesOrNoEnum.YES));
        // 如果是自己给自己点赞，则不处理
        if (userId.equals(topic.getUserId())) {
            return;
        }
    ...
    public void cancelLike(Long topicId) {
        ...
        if (YesOrNoEnum.YES.equals(topicLike.getHasLike())) {
            chatTopicLikeService.updateById(new ChatTopicLike().setId(topicLike.getId()).setHasLike(YesOrNoEnum.NO));
        }
    }
    ...
    public void delReply(Long replyId) {
        ...
        // 帖主/自己发布的帖子
        chatTopicReplyService.updateById(new ChatTopicReply().setReplyId(replyId).setReplyStatus(YesOrNoEnum.NO));
    }

    /**
     * 执行添加
     */
    private void addNotice(Long userId, ChatTopic topic, ChatUser fromUser, TopicNoticeTypeEnum noticeType, String content) {
